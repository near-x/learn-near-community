...
Solana使用历史证明算法，通过创造历史记录来证明某一事件发生在某一特定时刻。以下要点需要记住:
...
### 在 Solana 中编程
Solana 中的智能合约是用 Rust 或 c 编写的，并编译成 BPF/字节码(BPF)。由于有更多的工具可用，建议您在 Rust 中编写代码。初学者应该使用 Anchor 框架编写程序代码，该框架可以简化执行。
